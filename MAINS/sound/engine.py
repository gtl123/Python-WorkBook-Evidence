...
class Engine:
    ...
    def _gen_audio_one_engine_cycle(self):
        ...
        bufs = []
        bufsunequal = []
        fire_snd = slice(self.fire_snd, fire_duration)
        for cylinder in range(0, self.cylinders):
            ...
            before_fire_duration = (self.timing[
                                        cylinder] / 180) / strokes_per_sec  # + unequalms # 180 degrees crankshaft rotation per stroke
            before_fire_snd = slice(self.between_fire_snd, before_fire_duration + unequalms)
            after_fire_duration = between_fire_duration - before_fire_duration  # - unequalms
            after_fire_snd = slice(self.between_fire_snd, after_fire_duration)
