...
class UtilsGaurav:  # creates class Utils_Gaurav
    def __init__(self, ai, scratch):  # tells the program what to do when the Utils_Gaurav class has been initialised
        ...
        if scratch == True:
            ...
            self.user = scratchconnect.ScratchConnect("Insert user", "insert password")  # logs into a Scratch User to Give us access to all Scratch Features
    ...
    def INPUT(self, prompt, types, data, func):  # Input function with  4 parameters prompt,type and data + self
        ...
        elif types == "ME":  # Multiple Entries
            print(str(prompt))
            count = 0
            answers = []
            result = 0
    ...
    def add_list(self, answers):
        result = 0
        for answer in answers:
            result += int(answer)
        return result

    def minus_list(self, answers):
        result = 0
        if len(answers) > 1:
            result = int(answers[0])
            for answer in answers:
    ...
    def Country_Guessing_game(self):
        ...
        idx = randint(0, 2)
        while True:
            answer = self.INPUT(f"What is {selected_countries[idx]}'s capital ?: ", "MO", selected_capitals,self.NONE)
            try:
                int(answer)
            except ValueError:
