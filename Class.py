     ...
     if self.Ai_Enabled == True:
        try:  # Tries the following code check for errors that might be raised
            with sr.Microphone() as source2:  # calls the Microphone() function /attribute as keyword used to give a temporary name for example source2
                self.r.adjust_for_ambient_noise(source2, duration=0.2)  # calls the adjust_for_ambient_noise function with 2 arguments microphone source ,duration
                audio2 = self.r.listen(source2)  # calls the listen function with 1 argument microphone also known as source2 and is set to the variable assigned audio2
                MyText = self.r.recognize_google(audio2)  # calls the recognize_google function with one argument which is the sound input this function coverts the sound input into text using the Google Ai model and sets the variable assigned Mytext
        ...
        if self.Ai_Enabled == True and self.Scratch_Enabled == True :
          ...
          def set(data):  # function called set with 1 argument data
            if str(data["user"]) != "__BIT__":  # an if statement that activates the tabbed code under if the statement is true
                ...
                function = self.find("function", message)  # sets variable assigned function to the return value form the find function with string "function" and message given as parameter
                if function == "stream":  # an if statement that activates the tabbed code under if the statement is true
                    new_data = self.find("DATA", message)  # sets variable assigned new_data to the return value form the find string "DATA" and message given as parameter
    ...
    def Gaurav(self):  # function named Gaurav with 1 parameter self
     if self.Ai_Enabled ==True:
        while True:  # while statement set default to true meaning it will run infinite times
            txt = str(self.receive())  # sets the variable assigned txt to the string from return value from the receive() function
...
class Player(UtilsGaurav):
    ...
    def __init__(self):
        ...
        self.HP = 100
        self.Inventory = ["dagger", "sword", "wand", "Apples"]
        self.Item_Type = ["Weapon", "Weapon", "Weapon", "Food"]
        self.Item_Damage = ["10" , "35", "60", "0"]
        self.Item_Description = ["A dagger is a short, pointed knife that is sharp on both sides, used especially in the past as a weapon. A dagger is typically designed or capable of being used as a thrusting or stabbing weapon. Daggers have been used throughout human history for close combat confrontations, and many cultures have used adorned daggers in ritual and ceremonial contexts1. A dagger may have different materials, shapes and decorations depending on its origin and purpose.",
                                 "A sword is a bladed weapon intended for cutting or thrusting that is longer than a knife or dagger. A sword consists of a long blade attached to a hilt, which is usually equipped with a guard. A sword can be straight or curved, and can vary in length, breadth and configuration depending on the historical epoch or the geographic region under consideration14. A sword is often used as a symbol of honor or authority, and has been a preeminent hand weapon through a long period of history.",
                                 "A wand is a thin, light-weight rod that is held with one hand, and is traditionally made of wood, but may also be made of other materials, such as metal or plastic1. A wand has a magical substance at its core, such as phoenix feather, unicorn hair or dragon heartstring. Wands are used by witches and wizards to channel their magical powers and cast spells"]

    def process(self, attacker_name, attacker_HP, attacker_strengh, attacker_Emotional_attachment):
        option = self.INPUT("What do you want to do?", "MO", ["RUN", "FIGHT", "Declare Truce"])
        if option == "1":
            if attacker_strengh > self.strength:
                Metres = self.strength * self.Agility
                self.Print(f"you Run for your life as the {attacker_name} chases you sadly after {Metres} metres your energy depletes and the {attacker_name} is behind you ", 0.01)
            ...
            idx = self.Inventory.index(Weapons[option2])
            Weapon_damage = self.Item_Damage[idx]
            Multipliers = [self.Headshot_Multi, self.Torso_Multi, self.Legs_Multi]
            Multi = Multipliers[self.Roll(3)]
            if Weapon_damage*Multi > attacker_HP:
                if Multi == self.Legs_Multi:
                    self.Print(f"You use the {Weapons[option2]} and manage to  hit his legs ",0.2)
                    self.Print(f"The {attacker_name} collapsed to the ground, clutching his legs. ", 0.3)
                    self.Print(f"The pain was unbearable, like a thousand knives stabbing him at once. ", 0.3)
                    self.Print(f"He felt blood gushing from his wounds, soaking his clothes and the dirt beneath him. ", 0.3)
                    self.Print(f"He tried to scream, but no sound came out. ", 0.3)
                    self.Print(f"His vision blurred and he felt dizzy. ", 0.3)
                    self.Print(f"He knew he was dying, but he couldn’t accept it. ", 0.3)
                    self.Print(f"He had so much to live for, so much to do. ", 0.3)
                    self.Print(f"He wanted to see his family again, to tell them he loved them. ", 0.3)
                    self.Print(f"He wanted to fight for his cause, to make a difference in the world. ", 0.3)
                    self.Print(f"He wanted to live. But all he could do was lie there, helpless and hopeless, waiting for the end.", 0.3)
                    return {"status": "dead" , "HP": 0 , "Damage": Weapon_damage , "Multi": Multi , "loc": "legs"}
                elif Multi == self.Torso_Multi:
                    self.Print(f"You use the {Weapons[option2]} and manage to  hit his body ", 0.2)
                    self.Print("He felt a sudden impact in his chest, followed by a sharp pain that spread throughout his body. ", 0.3)
                    self.Print("He gasped for air, but it felt like he was breathing fire. ", 0.3)
                    self.Print("He looked down and saw blood pouring from his shirt. He realized he had been Hit.", 0.3)
                    self.Print("he staggered backwards, trying to reach for his secret weapon. But his arms were numb and heavy. ", 0.3)
                    self.Print("He fell to his knees, feeling weak and cold. He heard voices around him, shouting and shooting. ", 0.3)
                    self.Print("But they sounded distant and muffled. He closed his eyes, hoping it was all a nightmare. ", 0.3)
                    self.Print("But he knew it was real. He knew he was dying. And he was afraid.", 0.3)
                    return {"status": "dead" , "HP": 0 , "Damage": Weapon_damage , "Multi": Multi , "loc": "torso"}
                elif Multi == self.Headshot_Multi:
                    self.Print(f"You use the {Weapons[option2]} and manage to  hit his head ", 0.2)
                    self.Print("He heard a loud bang, and then everything went black. ", 0.3)
                    self.Print("He felt a brief flash of pain in his head, like a hammer smashing his skull. ", 0.3)
                    self.Print("He didn’t have time to react or think. He was dead before he hit the ground.", 0.3)
                    self.Print(" His brain was shattered by the bullet that pierced his temple. ", 0.3)
                    self.Print("His life was over in an instant. He had no memories, no regrets, no dreams.", 0.3)
                    self.Print(" He had nothing. He was nothing.", 0.3)
                    return {"status": "dead" , "HP": 0 , "Damage": Weapon_damage , "Multi": Multi , "loc": "head"}
                else:
                    raise IOError("UNKNOWN body HIT")
            else:
                self.Print(f"The {attacker_name} has taken {Weapon_damage*Multi} damage",0.2)
                return {"status": "engaged" , "HP": attacker_HP - (Weapon_damage * Multi) , "Damage": Weapon_damage , "Multi": Multi , "loc": "body"}
        elif option == "3":
            ...
            if Probability_of_acceptance == 1:
                ...
                print("DIEEE DIEEE!!")
                return {"status": "engaged", "HP": attacker_HP, "Damage": 0, "Multi": 0 , "loc": "body"}
        else:
            raise IOError("Not an Option")


class Monster(UtilsGaurav):

    def __init__(self, _name):
        super().__init__(ai=False,scratch=False)
        self.Monster_HP = self.Roll(20) + 5
        self.Headshot_Multi = 1.7
        self.Torso_Multi = 1.2
        self.Legs_Multi = 0.7
        self.name = _name
        self.Monster_Emotional_Attachment = self.Roll(20)
        self.Monster_Strengh = self.Roll(20)+10
        self.Inventory = ["dagger", "sword", "wand"]
        self.Item_Damage = ["10", "35", "60", "0"]

    def process(self, player_armourClass, player_HP):
        option2 = randint(1,len(self.Inventory)-1)
        idx = self.Inventory.index(self.Inventory[option2])
        Weapon_damage = self.Item_Damage[idx]
        Multipliers = [self.Headshot_Multi, self.Torso_Multi, self.Legs_Multi]
        Multi = Multipliers[self.Roll(3)]
        if Weapon_damage * Multi > player_HP:
            if Multi == self.Legs_Multi:
            ...
            self.Print(f"YOU have taken {Weapon_damage * Multi} damage", 0.2)
            return {"status": "engaged" , "HP": player_HP - (Weapon_damage * Multi) , "Damage": Weapon_damage , "Multi": Multi , "loc": "body"}